...
class Extent2UTMGrid(QgsProcessingAlgorithm):
    ...
    def ChartSize(self, geom, zone, hemisf, escala):
        ...
        projectedGeom = self.reprojectPoints(geom, coordinateTransformer)
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if not ProjectCRS.isGeographic():
            ...
            MinPoint = self.reprojectPoints(QgsGeometry(QgsPoint(x_min, y_min)), coordinateTransformer)
            MaxPoint = self.reprojectPoints(QgsGeometry(QgsPoint(x_max, y_max)), coordinateTransformer)
        ...
        for lon in LON:
            for lat in LAT:
                ...
                if tam_carta:
                    ...
                    feat[self.tr('height', 'altura')] = float(self.ChartSize(geom, zone, hemisf, escala)[0])
